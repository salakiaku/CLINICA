...
{
    public class DeletePatientHandler : IRequestHandler<DeletePatientRequetCommand, BaseResponse<bool>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IGenericRepository<Patient> _repository;

        public DeletePatientHandler(IUnitOfWork unitOfWork, IGenericRepository<Patient> repository)
        {
            _unitOfWork = unitOfWork; 
            _repository = repository;
        }

        public async Task<BaseResponse<bool>> Handle(DeletePatientRequetCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                var domainSuccess = await _repository.ExecAsync(STP.STPPatientsDelete, new { Id = request.Id },_unitOfWork.Transaction);
